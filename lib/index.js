...
const DEFAULT_OUTPUT = 32e3;
const name = "dsh-agent-browser";
const inject = ["tools"];
...
function sessionName(config, session) {
	const clean = session?.trim();
	if (session !== void 0 && !clean) throw new Error("session must be non-empty when provided");
	return clean ? `${config.sessionPrefix}-${clean}` : config.sessionPrefix;
}
function commandFor(tool, args, config) {
	const global = [
		"--session",
		sessionName(config, args.session),
		"--json",
		"--max-output",
		String(config.maxOutputChars),
		...config.screenshotDir ? ["--screenshot-dir", config.screenshotDir] : []
	];
	switch (tool) {
		case "browser_open": return [
			...global,
			"open",
			...args.url ? [args.url] : []
		];
		case "browser_snapshot": return [...global, "snapshot"];
		case "browser_read": return [
			...global,
			"read",
			...args.url ? [args.url] : []
		];
		...
		case "browser_fill": return [
			...global,
			"fill",
			args.selector,
			args.text
		];
		case "browser_type": return [
			...global,
			"type",
			args.selector,
			args.text
		];
		case "browser_press": return [
			...global,
			"press",
			args.key
		];
		...
		case "browser_eval": return [
			...global,
			"eval",
			args.expression
		];
		case "browser_get": return [
			...global,
			"get",
			args.target,
			...args.selector ? [args.selector] : []
		];
		case "browser_wait": return [
			...global,
			"wait",
			args.value
		];
...
async function runBrowserCommand(tool, args, config) {
	const command = commandFor(tool, args, config);
	try {
		const result = await new Promise((resolve, reject) => {
			childProcess.execFile(config.executablePath, command, {
				timeout: config.timeoutMs,
				maxBuffer: config.maxOutputChars * 2,
				windowsHide: true
			}, (error, stdout$1, stderr) => {
		...
		return {
			ok: true,
			command,
			session: sessionName(config, args.session),
			data,
			stdout,
			stderr: result.stderr.toString().slice(0, config.maxOutputChars),
			exitCode: 0,
			timedOut: false
		};
	} catch (error) {
		...
		return {
			...
			stdout,
			stderr,
			exitCode: failure.code && typeof failure.code === "number" ? failure.code : null,
...
const specs = [
	{
		name: "browser_open",
		description: "Launch the browser, optionally navigating to a URL.",
		parameters: {
			url: { type: "string" },
			session: { type: "string" }
		}
	},
	{
		...
		description: "Return the accessibility tree of the active page.",
		parameters: { session: { type: "string" } }
	},
	{
		name: "browser_read",
		description: "Read agent-friendly page text, or the active rendered page.",
		parameters: {
			url: { type: "string" },
			session: { type: "string" }
		}
	...
	{
		name: "browser_click",
		description: "Click an element by selector or accessibility ref.",
		parameters: {
			selector: {
				type: "string",
				required: true
	...
	{
		name: "browser_eval",
		description: "Evaluate JavaScript in the active page. Treat expression as untrusted page code.",
		parameters: {
			expression: {
				type: "string",
				required: true
	...
	{
		name: "browser_get",
		description: "Get URL, title, text, or HTML from the page.",
		parameters: {
			target: {
				type: "string",
				required: true,
	...
	{
		name: "browser_wait",
		description: "Wait for a selector or a number of milliseconds.",
		parameters: {
			condition: {
				type: "string",
				required: true,
				enum: ["selector", "timeout"]
			},
			value: {
				type: "string",
				required: true
...
function apply(ctx, config) {
	...
	for (const spec of specs) ctx.tools.register(defineTool({
		name: spec.name,
		...
		execute: (args) => runBrowserCommand(spec.name, args, resolved)
	}));
}
